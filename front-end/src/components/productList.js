            ...
            </ul>
            {
                products.length > 0 ? products.map((item, index) =>
                    <ul key={item._id}>
                        ...
                        <li>${item.price}</li>
                        <li>{item.category}</li>
                        <li><button className="badge rounded-pill text-bg-danger" onClick={() => deleteProduct(item._id)}>Delete</button>
                            <Link className="badge rounded-pill text-bg-success" to={"/update/" + item._id}>Update</Link>
                        </li>
